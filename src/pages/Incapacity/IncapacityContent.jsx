...
export default function IncapacityContent() {
...
const obtenerListadoGroups = async () => {
  ...
  const traerPermisosTrabajador = async ()=>{
    return await axios
    .get("http://localhost:3000/api/incapacitys/"+idUsuario,
    {
      params:{
...
const agregar = async ()=>{
  ...
  if(nameUser=="-----------------------")
  {
    toast.error("para agregar una incapacidad a un trabajador, primero seleccionelo de la lista");
  }
  else if(namePermissionRef.current.value=="")
  {
    namePermissionRef.current.focus();
    toast.error("Debe especificar un nombre que describa la incapacidad");
  }
  else if(fechaInicialRef.current.value=="")
  {
    fechaInicialRef.current.focus();
    toast.error("Debe especificar una fecha de inicio de la incapacidad");
  }
  else if(fechaFinalRef.current.value=="")
  {
    fechaFinalRef.current.focus();
    toast.error("Debe especificar una fecha de finalización de la incapacidad");
  }
  else if(observacionesRef.current.value=="")
  {
    observacionesRef.current.focus();
    toast.error("Debe especificar una obserbación o descripción de la incapacidad");
  }

  else
  {
    await axios.post("http://localhost:3000/api/incapacitys/", {
...
const eliminar = ()=>
{
  ...
  {
    Swal.fire({
      title: "¿Realmente desea eliminar el registro de incapacidad para " + nameUser + "?",
      showDenyButton: true,
...
const EliminarPermiso = async ()=>{
  await axios.delete("http://localhost:3000/api/incapacitys/" + idRegistro);
...
const editar = async ()=>{
 ...
 {
   namePermissionRef.current.focus();
   toast.error("El campo nombre del la incapacidad no puede estar vacio, seleccionelo de la lista de registros");
 }
 else if(fechaInicialRef.current.value=="")
 {
   fechaInicialRef.current.focus();
   toast.error("Debe especificar una fecha de inicio para la incapacidad");
 }
 else if(fechaFinalRef.current.value=="")
 {
   fechaFinalRef.current.focus();
   toast.error("Debe especificar una fecha de finalización para la incapacidad");
 }
 else if(observacionesRef.current.value=="")
 {
   observacionesRef.current.focus();
 ...
 else
 {
   ...
   await axios.put("http://localhost:3000/api/incapacitys/"+ idRegistro, {
     nombre: namePermissionRef.current.value,
     año:starIn.getFullYear().toString(),
     fechainicio: starIn,
     fechafinal: starOut,
  ...
  return (
    <div className="contentRotations">
      ...
      <img src={avatarUser} alt="Permissions" className="imgAvatar"/>
        ...
        <div className="mainCajas">
          <div className="subTitulo">Registro  de incapacidades durante el año {startDate.getFullYear()}</div>
      ...
      <div className="mainCajas2">
          ...
          <div className="caja1">
              <h5 className='titleDetail'>Nombre de la incapacidad</h5>
