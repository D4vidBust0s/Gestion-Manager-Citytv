...
const obtenerListadoGroups = async () => {
  ...
  const traerPermisosTrabajador = async ()=>{
    return await axios
    .get("http://localhost:3000/api/breaks/"+idUsuario,
    {
      params:{
...
const agregar = async ()=>{
  ...
  if(nameUser=="-----------------------")
  {
    toast.error("para agregar un registro de descanso al trabajador, primero seleccionelo de la lista");
  }
  else if(namePermissionRef.current.value=="")
  {
    namePermissionRef.current.focus();
    toast.error("Debe especificar un nombre que describa el motivo que describa el descanso que  toma el trabajador");
  }
  else if(fechaInicialRef.current.value=="")
  {
    fechaInicialRef.current.focus();
    toast.error("Debe especificar una fecha de inicio del descanso");
  }
  ...
  else if(observacionesRef.current.value=="")
  {
    ...
    toast.error("Debe especificar una obserbación o descripción para el descanso");
  }
...
const eliminar = ()=>
{
  ...
  {
    Swal.fire({
      title: "¿Realmente desea eliminar el registro de descanso para " + nameUser + "?",
      showDenyButton: true,
...
const EliminarPermiso = async ()=>{
  await axios.delete("http://localhost:3000/api/breaks/" + idRegistro);
...
const editar = async ()=>{
 ...
 {
   namePermissionRef.current.focus();
   toast.error("El campo nombre del descanso no puede estar vacio, seleccionelo de la lista de registros");
 }
 else if(fechaInicialRef.current.value=="")
 {
   fechaInicialRef.current.focus();
   toast.error("Debe especificar una fecha de inicio para el descanso");
 }
 else if(fechaFinalRef.current.value=="")
 {
   fechaFinalRef.current.focus();
   toast.error("Debe especificar una fecha de finalización para el descanso");
 }
 else if(observacionesRef.current.value=="")
 {
   observacionesRef.current.focus();
 ...
 else
 {
   ...
   await axios.put("http://localhost:3000/api/breaks/"+ idRegistro, {
     nombre: namePermissionRef.current.value,
     año:starIn.getFullYear().toString(),
     fechainicio: starIn,
     fechafinal: starOut,
  ...
  return (
    <div className="contentRotations">
      ...
      <img src={avatarUser} alt="Permissions" className="imgAvatar"/>
        ...
        <div className="mainCajas">
          <div className="subTitulo">Registro de descansos durante el año {startDate.getFullYear()}</div>
      ...
      <div className="mainCajas2">
          ...
          <div className="caja1">
              <h5 className='titleDetail'>Nombre descriptivo del descanso</h5>
