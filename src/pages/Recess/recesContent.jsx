...
const obtenerListadoGroups = async () => {
  ...
  const traerPermisosTrabajador = async ()=>{
    return await axios
    .get("http://localhost:3000/api/recess/"+idUsuario,
    {
      params:{
...
const agregar = async ()=>{
  ...
  if(nameUser=="-----------------------")
  {
    toast.error("para agregar un registro de vacaciones al trabajador, primero seleccionelo de la lista");
  }
  else if(namePermissionRef.current.value=="")
  {
    namePermissionRef.current.focus();
    toast.error("Debe especificar un nombre que describa el periodo de vacaciones que toma el trabajador");
  }
  else if(fechaInicialRef.current.value=="")
  {
    fechaInicialRef.current.focus();
    toast.error("Debe especificar una fecha de inicio del periodo de vacaciones");
  }
  else if(fechaFinalRef.current.value=="")
  {
    fechaFinalRef.current.focus();
    toast.error("Debe especificar una fecha de finalización del periodo de vacaciones");
  }
  else if(observacionesRef.current.value=="")
  {
    observacionesRef.current.focus();
    toast.error("Debe especificar una obserbación o descripción del periodo de vacaciones");
  }

  else
  {
    await axios.post("http://localhost:3000/api/recess/", {
...
const eliminar = ()=>
{
  ...
  {
    Swal.fire({
      title: "¿Realmente desea eliminar el registro de vacaciones para " + nameUser + "?",
      showDenyButton: true,
...
const EliminarPermiso = async ()=>{
  await axios.delete("http://localhost:3000/api/recess/" + idRegistro);
...
const editar = async ()=>{
 ...
 {
   namePermissionRef.current.focus();
   toast.error("El campo nombre del periodo de vacaciones no puede estar vacio, seleccionelo de la lista de registros");
 }
 else if(fechaInicialRef.current.value=="")
 {
   fechaInicialRef.current.focus();
   toast.error("Debe especificar una fecha de inicio para el periodo de vacaciones");
 }
 else if(fechaFinalRef.current.value=="")
 {
   fechaFinalRef.current.focus();
   toast.error("Debe especificar una fecha de finalización para el periodo de vacaciones");
 }
 else if(observacionesRef.current.value=="")
 {
   observacionesRef.current.focus();
   toast.error("Debe especificar una obserbación o descripción para el periodo de vacaciones");
 }

 else
 {
  
   await axios.put("http://localhost:3000/api/recess/"+ idRegistro, {
     nombre: namePermissionRef.current.value,
     año:starIn.getFullYear().toString(),
     fechainicio: starIn,
     fechafinal: starOut,
  ...
  return (
    <div className="contentRotations">
      ...
      <img src={avatarUser} alt="Permissions" className="imgAvatar"/>
        ...
        <div className="mainCajas">
          <div className="subTitulo">Registro de vacaciones durante el año {startDate.getFullYear()}</div>
      ...
      <div className="mainCajas2">
          ...
          <div className="caja1">
              <h5 className='titleDetail'>Nombre del periodo de vacaciones</h5>
